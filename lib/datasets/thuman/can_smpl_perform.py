...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #### sanit check ####
        # smpl_obj_path = os.path.join(cfg.virt_data_root, 'smplx_obj', f'{human}.obj')
        # smpl_obj = readobj(smpl_obj_path)   # expects readobj to return {'vi': verts}
        # verts = smpl_obj['vi'].astype(np.float32).T
        # #verts = smpl_obj['vi'].astype(np.float32).T  # [3, 6890]

        # # Project vertices to image space using P = K(RX + T)
        # proj = K @ (R @ verts + T.reshape(3, 1))    # shape: (3, N)
        # proj = proj[:2] / proj[2:]                  # normalize
        # uv = proj.T.astype(np.int32)                # (N, 2)

        # # Filter valid points (in-bounds)
        ...
        # # Draw each point
        # for x, y in uv:
        #     cv2.circle(img_vis, (x, y), radius=1, color=(0, 255, 0), thickness=-1)

        # # Save or display
        # cv2.imwrite("projected_overlay.png", img_vis)

        # # ---- Load novel view image ----
        # novel_cam_id = 12  # or whichever camera index you want as novel view
        # cam_str = f"{human}_{novel_cam_id:03d}"
        # img_path = os.path.join(cfg.virt_data_root, 'img', cam_str, f'{frame}.jpg')
        ...
        # obj_path = os.path.join(cfg.virt_data_root, 'smplx_obj', f'{human}.obj')
        # smpl_obj = readobj(obj_path)
        # verts = smpl_obj['vi'].astype(np.float32).T  # [3, N]

        # # ---- Project onto 2D ----
        # proj = K @ (R @ verts + T.reshape(3, 1))  # shape (3, N)
        # proj = proj[:2] / proj[2:]                # normalize
        # uv = proj.T.astype(np.int32)              # shape (N, 2)
        ...
        # for x, y in uv:
        #     cv2.circle(img_vis, (x, y), radius=1, color=(0, 0, 255), thickness=-1)

        # # ---- Save ----
        # cv2.imwrite("overlay_novel_view.png", img_vis)
        # pdb.set_trace()
        ####### end #######
